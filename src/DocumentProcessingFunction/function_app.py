...
def Loaders(myblob: func.InputStream):
    ...
    if not myblob.name.lower().endswith('.json'):
        return f"Skipping processing: {myblob.name} is not a json file."
...
class DocumentLoader:
    ...
    def _create_document(self,page:dict, resource_id:str, title:str):
        document = Document(
    ...
    def load_chunk_document(self,
                            chunk_size=2000, 
                    chunk_overlap=500,
                    length_function=len,
                    is_separator_regex=False) -> List[Document]:
        
        logging.info(f"Load and return documents from blob.")
        ...
        if not index_exists:
            ...
            index = SearchIndex(
                ...
                fields=[
                    ...
                    SearchableField(name="chapter", type="Edm.String", filterable=True, sortable=True),
                    SearchableField(name="resource_id", type="Edm.String", filterable=False, sortable=False),                    
        ...
        try:
            ...
            for index, chunk in enumerate(chunks):
                try:
                    ...
                    page_number = str(chunk.metadata["pagenumber"])
                    chapter = str(chunk.metadata["chapter"]) 
                    resource_id = str(chunk.metadata["resource_id"])  
